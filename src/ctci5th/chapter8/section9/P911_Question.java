...
/**
 * 给出一个布尔表达式, 由0, 1, & | , ()等组成，　以及一个想要的结果
 * 实现一个函数, 算出有几种括号的放法可使的这个表达式得出result值.
 *
 * f(1^0|0|1, true) -- 所有值为result的expr的数目.
 * f(1^0|0|1, true) = f((1^0)|(0|1), true) + f((1^0|0)|1, true) + f(1^(0|0|1), true)
 *
 * f(expr1 | expr2, true) = f(expr1, true) * f(expr2, true)
 * 												+ f(expr1, false) * f(expr2, true)
 * 											  + f(expr1, true) * f(expr2, false)
 * f(expr1 & expr2, true) = f(expr1, true) * f(expr2, true)
 * f(expr1 ^ expr2, true) = f(expr1, true) * f(expr2, false)
 * 												+ f(expr1, false) * f(expr2, true)
 *
 *为false的情况一样, 就不列举了.
 */
public class P911_Question {
	public enum Term {
		True,
		False,
		And,
