...
public class HeapSort {
  ...
  public static void main(String args[]) {
    ...
    int arrayNumber = 1500;
    int[] integers = new int[arrayNumber];
    ...
    HeapSort hs = new HeapSort();
    hs.heapSort(integers);
    /*hs.toString(integers);*/
    boolean result = hs.check(integers);
    if (result) {
  ...
  private void test() {
    ...
    for (int i = 0; i < times; i++) {
      int[] ints = new int[base * (int) Math.pow(2, i)];
      int max = base * (int) Math.pow(2, i + 1);
      for (int j = 0; j < ints.length; j++) {
        ints[j] = random.nextInt(max);
      }
  ...
  private boolean check(int[] integers) {
    for (int i = 0; i < integers.length - 1; i++) {
      if (integers[i] > integers[i + 1]) {
        System.out.println("i = " + i + ", i+1=" + (i + 1) + "," + integers[i] + ", " + integers[i + 1]);
        return false;
      }
    }
    return true;
