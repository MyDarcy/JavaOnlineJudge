...
public class P1_Main2 {
  ...
  public static void main(String[] args) {
    ...
    Bear[] bearForIndex = new Bear[bearNumber];
    while (i < bearNumber) {
      Bear bear = new Bear();
    ...
    List<Bear> list = solution(bears, candyEnergy);
    /*for (Integer item : list) {
      System.out.println(item);
    }*/


    for (int j = 0; j < bearForIndex.length; j++) {
      for (Bear bear : list) {
        if (bearForIndex[j] == bear) {
  ...
  private static List<Bear> solution(List<Bear> bears, int[] candyEnergy) {
    boolean[] candyEat = new boolean[candyEnergy.length];

    List<Bear> leftEnergy = new LinkedList<>();
